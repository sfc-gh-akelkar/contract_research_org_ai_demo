...
SELECT 
    ...
    ROUND(2.5 + (ABS(RANDOM()) * 2.0), 2) AS HISTORICAL_ENROLLMENT_RATE,
    -- High data quality: 85-98
    ROUND(85 + (ABS(RANDOM()) * 13), 1) AS DATA_QUALITY_SCORE,
    -- Experienced investigators: 8-25 years
    8 + FLOOR(ABS(RANDOM()) * 18) AS INVESTIGATOR_YEARS_EXPERIENCE,
    -- High compliance: 88-99
    ROUND(88 + (ABS(RANDOM()) * 11), 1) AS REGULATORY_COMPLIANCE_SCORE,
    -- Low screen failure: 0.05-0.15
    ROUND(0.05 + (ABS(RANDOM()) * 0.10), 4) AS SCREEN_FAILURE_RATE,
    -- Low protocol deviations: 0.01-0.05
    ROUND(0.01 + (ABS(RANDOM()) * 0.04), 4) AS PROTOCOL_DEVIATION_RATE,
    -- Few critical findings: 0-2
    FLOOR(ABS(RANDOM()) * 3) AS CRITICAL_FINDINGS_COUNT,
    -- High retention: 0.88-0.98
    ROUND(0.88 + (ABS(RANDOM()) * 0.10), 4) AS PATIENT_RETENTION_RATE,
    'High' AS PERFORMANCE_CATEGORY,
    ROUND(2.8 + (ABS(RANDOM()) * 1.5), 2) AS PREDICTED_ENROLLMENT_RATE
...
SELECT 
    ...
    ROUND(1.2 + (ABS(RANDOM()) * 1.6), 2) AS HISTORICAL_ENROLLMENT_RATE,
    -- Medium data quality: 70-88
    ROUND(70 + (ABS(RANDOM()) * 18), 1) AS DATA_QUALITY_SCORE,
    -- Mixed experience: 3-15 years
    3 + FLOOR(ABS(RANDOM()) * 13) AS INVESTIGATOR_YEARS_EXPERIENCE,
    -- Medium compliance: 75-90
    ROUND(75 + (ABS(RANDOM()) * 15), 1) AS REGULATORY_COMPLIANCE_SCORE,
    -- Medium screen failure: 0.12-0.25
    ROUND(0.12 + (ABS(RANDOM()) * 0.13), 4) AS SCREEN_FAILURE_RATE,
    -- Medium protocol deviations: 0.03-0.08
    ROUND(0.03 + (ABS(RANDOM()) * 0.05), 4) AS PROTOCOL_DEVIATION_RATE,
    -- Some critical findings: 1-4
    1 + FLOOR(ABS(RANDOM()) * 4) AS CRITICAL_FINDINGS_COUNT,
    -- Medium retention: 0.75-0.90
    ROUND(0.75 + (ABS(RANDOM()) * 0.15), 4) AS PATIENT_RETENTION_RATE,
    'Medium' AS PERFORMANCE_CATEGORY,
    ROUND(1.5 + (ABS(RANDOM()) * 1.2), 2) AS PREDICTED_ENROLLMENT_RATE
...
SELECT 
    ...
    ROUND(0.3 + (ABS(RANDOM()) * 1.2), 2) AS HISTORICAL_ENROLLMENT_RATE,
    -- Lower data quality: 55-75
    ROUND(55 + (ABS(RANDOM()) * 20), 1) AS DATA_QUALITY_SCORE,
    -- Less experience: 1-8 years
    1 + FLOOR(ABS(RANDOM()) * 8) AS INVESTIGATOR_YEARS_EXPERIENCE,
    -- Lower compliance: 60-80
    ROUND(60 + (ABS(RANDOM()) * 20), 1) AS REGULATORY_COMPLIANCE_SCORE,
    -- Higher screen failure: 0.20-0.40
    ROUND(0.20 + (ABS(RANDOM()) * 0.20), 4) AS SCREEN_FAILURE_RATE,
    -- Higher protocol deviations: 0.06-0.15
    ROUND(0.06 + (ABS(RANDOM()) * 0.09), 4) AS PROTOCOL_DEVIATION_RATE,
    -- More critical findings: 3-8
    3 + FLOOR(ABS(RANDOM()) * 6) AS CRITICAL_FINDINGS_COUNT,
    -- Lower retention: 0.60-0.80
    ROUND(0.60 + (ABS(RANDOM()) * 0.20), 4) AS PATIENT_RETENTION_RATE,
    'Low' AS PERFORMANCE_CATEGORY,
    ROUND(0.5 + (ABS(RANDOM()) * 0.8), 2) AS PREDICTED_ENROLLMENT_RATE
...
UPDATE SITE_PERFORMANCE_FEATURES SET
    ENROLLMENT_EFFICIENCY = ROUND(
        LEAST(
            CASE 
                WHEN SCREEN_FAILURE_RATE <= 0.005 THEN HISTORICAL_ENROLLMENT_RATE / 0.01
                ELSE HISTORICAL_ENROLLMENT_RATE / SCREEN_FAILURE_RATE
            END, 
            9999.9999  -- Cap at maximum value for DECIMAL(8,4)
        ), 4),
