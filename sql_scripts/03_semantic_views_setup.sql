...
CREATE OR REPLACE SEMANTIC VIEW CRO_AI_DEMO.CLINICAL_OPERATIONS_SCHEMA.ML_ENHANCED_CLINICAL_VIEW
	tables (
		STUDIES as DIM_STUDIES primary key (STUDY_ID),
		SPONSORS as DIM_SPONSORS primary key (SPONSOR_ID),
		THERAPEUTIC_AREAS as DIM_THERAPEUTIC_AREAS primary key (THERAPEUTIC_AREA_ID),
		...
		SAFETY_EVENTS as FACT_SAFETY_EVENTS primary key (SAFETY_EVENT_ID),
		MONITORING as FACT_SITE_MONITORING primary key (MONITORING_VISIT_ID),
		ML_PREDICTIONS as CRO_AI_DEMO.ML_MODELS.ML_PREDICTIONS primary key (PREDICTION_ID),
		ENROLLMENT_FORECAST as ENROLLMENT_PERFORMANCE_FORECAST primary key (STUDY_TITLE, SITE_NAME),
		RISK_ALERTS as HIGH_RISK_SITES_ALERT primary key (SITE_NAME)
	)
	relationships (
		STUDYTOSPONSOR as STUDIES(SPONSOR_ID) references SPONSORS(SPONSOR_ID),
		STUDYTOTHERAPEUTIC as STUDIES(THERAPEUTIC_AREA_ID) references THERAPEUTIC_AREAS(THERAPEUTIC_AREA_ID),
	...
	facts (
		...
		MONITORING.PROTOCOL_DEVIATIONS_FOUND as protocol_deviations_found,
		MONITORING.CRITICAL_FINDINGS as critical_findings,
		ML_PREDICTIONS.PREDICTION_VALUE as ml_prediction_value,
		ML_PREDICTIONS.CONFIDENCE_SCORE as ml_confidence_score,
		ENROLLMENT_FORECAST.PREDICTED_ENROLLMENT_RATE as predicted_enrollment_rate,
		RISK_ALERTS.RISK_PROBABILITY as site_risk_probability
	)
	dimensions (
		STUDIES.STUDY_ID as study_id,
		STUDIES.PROTOCOL_NUMBER as protocol_number,
		...
		SAFETY_EVENTS.SEVERITY as severity,
		SAFETY_EVENTS.CAUSALITY as causality,
		ML_PREDICTIONS.PREDICTION_CATEGORY as ml_prediction_category,
		ML_PREDICTIONS.ENTITY_TYPE as ml_entity_type,
		ENROLLMENT_FORECAST.PERFORMANCE_CATEGORY as predicted_performance_category,
		RISK_ALERTS.PRINCIPAL_INVESTIGATOR as at_risk_investigator
	)
	metrics (
		STUDIES.TOTAL_ENROLLMENT as SUM(actual_enrollment)
			WITH SYNONYMS = ('total enrollment', 'total subjects', 'enrolled patients'),
		...
		SITES.AVG_DATA_QUALITY as AVG(data_quality_score)
			WITH SYNONYMS = ('data quality score', 'site performance', 'data quality'),
		ML_PREDICTIONS.HIGH_RISK_SITES as SUM(CASE WHEN ml_prediction_category = 'High Risk' THEN 1 ELSE 0 END)
			WITH SYNONYMS = ('high risk sites', 'sites at risk', 'underperforming sites'),
		ML_PREDICTIONS.PREDICTED_HIGH_PERFORMERS as SUM(CASE WHEN ml_prediction_category = 'High Performance Expected' THEN 1 ELSE 0 END)
			WITH SYNONYMS = ('high performing sites', 'top performing sites', 'predicted high performers'),
		ENROLLMENT_FORECAST.AVG_PREDICTED_ENROLLMENT as AVG(predicted_enrollment_rate)
			WITH SYNONYMS = ('predicted enrollment rate', 'forecasted enrollment', 'enrollment prediction'),
		ML_PREDICTIONS.AVG_ML_CONFIDENCE as AVG(ml_confidence_score)
			WITH SYNONYMS = ('ML confidence', 'prediction confidence', 'model confidence')
	);
...
GRANT USAGE ON SEMANTIC VIEW ML_ENHANCED_CLINICAL_VIEW TO ROLE SF_INTELLIGENCE_DEMO;
